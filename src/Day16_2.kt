fun main() {
    ...
    fun solve(i: Int, j: Int, d: Int): Int {
        for (i in 0..<n) for (j in 0..<m) v[i][j].fill(false)
        enq(i - di[d], j - dj[d], d)
        while (!q.isEmpty()) {
    ...
    val ans = listOf(
        (0..<n).maxOf { i -> solve(i, 0, 0) },
        (0..<m).maxOf { j -> solve(0, j, 1) },
        (0..<n).maxOf { i -> solve(i, m - 1, 2) },
        (0..<m).maxOf { j -> solve(n - 1, j, 3) },
    ).max()
    println(ans)
}